...
- **Sección Video** (modo `4`, vía fal.ai con una sola clave en `~/.fal_key`): **Seedance 2.0** y **Seedance 2.0 Fast** (texto→video e imagen→video, 480p–1080p, 4–15s o auto, 7 aspectos, audio nativo, seed), **Kling 3.0 Pro** (3–15s, prompt negativo, CFG, audio nativo, imagen inicial) y **OmniHuman 1.5** (avatar que habla: imagen de una persona + audio — puedes elegir directamente una voz generada en la sección Audio — con turbo y 720p/1080p a $0.14/s). Generación asíncrona con estado de cola en vivo; el MP4 se descarga solo al historial y a tu carpeta, con su sección de Video en el panel derecho (reproducir, descargar, borrar).
